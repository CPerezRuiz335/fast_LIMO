...
#define G_m_s2 (9.807)               // Gravaty const in GuangDong/China
#define MATCH_POINTS    (5)     
...
float calc_dist(MapPoint p1, MapPoint p2){
    float d = (p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y) + (p1.z - p2.z) * (p1.z - p2.z);
    return d;
}

template<typename T>
bool esti_plane(Matrix<T, 4, 1> &pca_result, const MapPoints &point, const T &threshold)
{
    Matrix<T, MATCH_POINTS, 3> A;
    Matrix<T, MATCH_POINTS, 1> b;
    A.setZero();
    b.setOnes();
    b *= -1.0f;
...
namespace esekfom
{
	...
	pcl::PointCloud<pcl::PointXYZI>::Ptr normvec(new pcl::PointCloud<pcl::PointXYZI>(100000, 1));		  //特征点在地图中对应的平面参数(平面的单位法向量,以及当前点到平面距离)
	pcl::PointCloud<MapPoint>::Ptr laserCloudOri(new pcl::PointCloud<MapPoint>(100000, 1)); //有效特征点
	pcl::PointCloud<pcl::PointXYZI>::Ptr corr_normvect(new pcl::PointCloud<pcl::PointXYZI>(100000, 1)); //有效特征点对应点法相量
	bool point_selected_surf[10000000] = {1};							  //判断是否是有效特征点
	...
	public:
		...
		//计算每个特征点的残差及H矩阵
		void h_share_model(dyn_share_datastruct &ekfom_data, pcl::PointCloud<MapPoint>::Ptr &feats_down_body,
						   KD_TREE<MapPoint>::Ptr ikdtree, bool extrinsic_est)
		{
			int feats_down_size = feats_down_body->points.size();

			ekfom_data.h_x = MatrixXd::Zero(feats_down_size, 12);
			ekfom_data.h = MatrixXd::Zero(feats_down_size, 1);

			#pragma omp parallel for num_threads(6)
			for (int i = 0; i < feats_down_size; i++) //遍历所有的特征点
			{
				MapPoint &point_body = feats_down_body->points[i];
				...
				vector<float> pointSearchSqDis(MATCH_POINTS);
				MapPoints points_near;

				ikdtree->Nearest_Search(point_world, MATCH_POINTS, points_near, pointSearchSqDis);
				point_selected_surf[i] = points_near.size() < MATCH_POINTS ? false : pointSearchSqDis.back() > 4 ? false
																										: true;
				if (not point_selected_surf[i])
					continue;

				Matrix<float, 4, 1> pabcd;		

				float pd2;
				if (esti_plane(pabcd, points_near, 5.e-2f))
				{
					pd2 = pabcd(0) * point_world.x + pabcd(1) * 
					      point_world.y + pabcd(2) * point_world.z + pabcd(3); 
				} else {
					continue;
				}
				...
				if (extrinsic_est)
				{
					...
					ekfom_data.h_x.block<1, 12>(i, 0) << norm_vec(0), norm_vec(1), norm_vec(2),  
		...
		void update_iterated_dyn_share_modified(double R,
												pcl::PointCloud<MapPoint>::Ptr &feats_down_body,
												KD_TREE<MapPoint>::Ptr ikdtree,
												int maximum_iter, bool extrinsic_est)
		{
			...
			for (int i = -1; i < 3; i++) // maximum_iter是卡尔曼滤波的最大迭代次数
			{
				dyn_share.valid = true;
				// 计算雅克比，也就是点面残差的导数 H(代码里是h_x)
				h_share_model(dyn_share, feats_down_body, ikdtree, extrinsic_est);
